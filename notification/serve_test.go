...
import (
	...
	_ "github.com/firetiger-oss/storage/notification/aws"
	_ "github.com/firetiger-oss/storage/notification/cloudflare"
	_ "github.com/firetiger-oss/storage/notification/gcp"
)
...
func TestServeHTTPModeWithRegistration(t *testing.T) {
	...
	go func() {
		errCh <- notification.Serve(objectHandler,
			notification.WithPort(strconv.Itoa(port)),
			notification.WithHealthPath("/health"),
		)
	}()
	...
	t.Run("AWS EventBridge endpoint registered via init", func(t *testing.T) {
		payload := `{
			"version": "0",
			"id": "test-id",
		...
		resp, err := http.Post(baseURL+"/aws", "application/json", strings.NewReader(payload))
		if err != nil {
	...
	t.Run("GCP Pub/Sub endpoint registered via init", func(t *testing.T) {
		payload := `{
			"message": {
				"data": "eyJuYW1lIjoidGVzdC50eHQiLCJidWNrZXQiOiJteS1idWNrZXQiLCJzaXplIjoiMTEifQ==",
				"attributes": {
					"bucketId": "my-bucket",
					"objectId": "test.txt",
		...
		resp, err := http.Post(baseURL+"/gcp", "application/json", strings.NewReader(payload))
		if err != nil {
	...
	t.Run("Cloudflare Queues endpoint registered via init", func(t *testing.T) {
		payload := `{
			"account": "account-id",
			"action": "PutObject",
			"bucket": "my-bucket",
		...
		resp, err := http.Post(baseURL+"/cloudflare", "application/json", strings.NewReader(payload))
		if err != nil {
...
func TestServeWithCustomOptions(t *testing.T) {
	bucket := memory.NewBucket()
	_, err := bucket.PutObject(t.Context(), "test.txt",
	...
	go func() {
		notification.Serve(objectHandler,
			notification.WithPort(strconv.Itoa(port)),
			notification.WithHealthPath("/custom-health"),
		)
	}()
...
func TestServeDisabledHealthEndpoint(t *testing.T) {
	bucket := memory.NewBucket()
	registry := storage.RegistryFunc(func(ctx context.Context, uri string) (storage.Bucket, error) {
		return bucket, nil
	})
	...
	go func() {
		notification.Serve(objectHandler,
			notification.WithPort(strconv.Itoa(port)),
			notification.WithHealthPath(""), // Disable health endpoint
		)
	}()
